...
class ViewController: UIViewController, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let photo = post.photos.first {
            ...
            let request = ImageRequest(
                url: imageUrl,
                processors: [ImageProcessors.Resize(width: 500)],
                priority: .high
            )
            ...
            ImagePipeline.shared.loadImage(with: request) { result in
                ...
                case .success(let response):
                    DispatchQueue.main.async {
                        cell.tumblrCellImage.image = response.image
                    }
    ...
    func fetchPosts() {
        let url = URL(string:
                        "https://api.tumblr.com/v2/blog/peacecorps/posts/photo?api_key=tRXIXW87WW0tHZer7kERBvi47xl3WD2pXuD4TlHbHnijxQdS5n")!
